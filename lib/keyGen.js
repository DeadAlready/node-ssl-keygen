...
KeyGen.prototype.createKey = function createKey(name, force, callback){
  ...
  fs.stat(pair.key, function(err, stat){
    ...
    } else if(force){ // Force creation
      exec(command, self._cC(callback, function(){
        _returnMap(callback, pair.key, true);
      }));
    } else { //Callback with map
...
KeyGen.prototype.createSignRequest = function createSignRequest(name, callback){
  ...
  this.createKey(name, this._cC(callback, function(){
    exec(utils.stringify('openssl req -new -subj :subj: -key :key: -out :csr:', pair), self._cC(callback, function(){
      _returnMap(callback, pair.csr, true);
    }));
  }));
}
 ...
 * @param {Function} callback
 */
KeyGen.prototype.createSelfSigned = function createSelfSigned(name, ca, force, callback){
  if(force instanceof Function){
    callback = force;
    force = false;
  }
  ...
  var self = this;
  self.createKey(name, ca.key._created || force, self._cC(callback, function(key){
...
/**
 * Function for returning foldermap.map object to callback
 * 
...
function _returnMap(callback, file, created){
  if(typeof file === 'string'){
    var map = foldermap.mapSync(file);
    Object.defineProperty(map, '_created', {value:created});
  } else {
    var map = {};
    for(var i in file){
      map[i] = foldermap.mapSync(file[i]);
